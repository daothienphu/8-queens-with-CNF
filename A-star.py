...
def display_board(seqs):
    ...
    print('The number of queens to attack is' + str(attacked_queens_pairs(seqs)) + "  f(n)= " +str(attacked_queens_pairs(seqs)+ next_row_invalids(seqs)))
rows = [0] * 8
cols = [0] * 8
diagonal_main = [0] * 15
diagonal_sub = [0] * 15
    ...
    if(len(LIST)>0):
        for j in range(len(nums)): # In the first position of 0 in the sequence, that is, the leftmost column without queen, select a row to place queen
            ...
            if ((diagonal_main[pos + temp ] ==0) & (diagonal_sub [temp- pos +7]==0) ):
                ...
                frontier_priority_queue.append({'unplaced_queens':temp_seqs.count(0), 'pairs':8*attacked_queens_pairs(temp_seqs) + next_row_invalids(temp_seqs),'seqs':temp_seqs})
